...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateStickyColumnWidths() {
        ...
        headers.forEach((header, index) => {
            if (index < 4) { // Assuming first 4 columns are sticky
                header.style.width = `${columnWidths[index]}px`;
            }
        });

        // Apply widths and positioning to body cells
        rows.forEach(row => {
            row.querySelectorAll('td').forEach((cell, index) => {
                if (index < 4) { // Assuming first 4 columns are sticky
                    cell.style.width = `${columnWidths[index]}px`;
                    ...
                    cell.style.left = `${columnWidths.slice(0, index).reduce((a, b) => a + b, 0)}px`;
                    cell.style.backgroundColor = 'black'; // Ensure cell background is white for visibility
                    cell.style.zIndex = '1'; // Ensure sticky columns appear on top
                }
            });
        });
    }

    // Initial update and resize event listener
    updateStickyColumnWidths();
    window.addEventListener('resize', updateStickyColumnWidths);
});
    ...
    <script>
        ...
        function sortTable(columnIndex) {
            ...
            // Update sticky column widths after showing all rows
        updateStickyColumnWidths();
        }
